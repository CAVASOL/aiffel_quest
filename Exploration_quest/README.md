...
`OpenPose 및 Canny`를 활용하여 인간의 행동과 경계가 감지된 형태를 담은 독특한 이미지를 생성했습니다. 이러한 이미지를 합쳐 새로운 시각 경험을 만들었는데, 움직임과 명확한 시각적 윤곽이 조합되었습니다. 또한, 기존에 작업했던 모델 중 하나를 선택하여 매개변수를 조정하여 성능을 최적화한 후, `TFServing 및 Docker`를 활용하여 Docker 컨테이너 내에서 모델을 배포했습니다.